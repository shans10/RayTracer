...
class yz_rect : public hittable {
    public:
        ...
        virtual bool bounding_box(double time0, double time1, aabb& output_box) const override {
            // The bounding box must have non-zero width in each dimension, so pad the X dimensoon a small amount
            output_box = aabb(point3(k-0.0001, y0, z0), point3(k+0.001, y1, z1));
            return true;
        }
